...
<img src="https://latex.codecogs.com/gif.latex?L_{R&space;A(X),T}&space;=&space;L_{A(X),T}&space;&plus;&space;(K_I,&space;K_T,&space;K_R,&space;K_S)" title="L_{R A(X),T} = L_{A(X),T} + (K_I, K_T, K_R, K_S)" />
